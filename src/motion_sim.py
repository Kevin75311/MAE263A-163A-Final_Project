...
def pattern_circle(N=30, r=0.7, cx=DEFAULT_CENTER_X, cy=DEFAULT_CENTER_Y):
    t = np.linspace(0, 2*np.pi, N)
    return r * np.cos(t) + cx, r * np.sin(t) + cy

def pattern_flower(N=50, r=0.7, petals=6, cx=DEFAULT_CENTER_X, cy=DEFAULT_CENTER_Y):
    t = np.linspace(0, 2*np.pi, N)
    rho = r * (0.5 + 0.5 * np.cos(petals * t))
    return rho * np.cos(t) + cx, rho * np.sin(t) + cy

def pattern_heart(N=40, scale=0.7, cx=DEFAULT_CENTER_X, cy=DEFAULT_CENTER_Y):
    t = np.linspace(0, 2*np.pi, N)
    x = scale * 16 * np.sin(t)**3 / 20
    y = scale * (13*np.cos(t) - 5*np.cos(2*t) - 2*np.cos(3*t) - np.cos(4*t)) / 20 + 0.4
    return x + cx, y + cy

def pattern_spirograph(R=1.0, r=0.4, p=0.6, N=100, cx=DEFAULT_CENTER_X, cy=DEFAULT_CENTER_Y):
    t = np.linspace(0, 30*np.pi, N)
    x = (R - r) * np.cos(t) + p * np.cos((R - r)/r * t)
    y = (R - r) * np.sin(t) - p * np.sin((R - r)/r * t)
    return x*0.6 + cx, y*0.6 + cy

def pattern_lissajous(a=3, b=4, delta=np.pi/2, scale=0.7, N=100, cx=DEFAULT_CENTER_X, cy=DEFAULT_CENTER_Y):
    t = np.linspace(0, 2*np.pi, N)
    x = scale * np.sin(a * t + delta)
    y = scale * np.cos(b * t)
    return x + cx, y + cy

def pattern_hypocycloid(petals=5, scale=0.7, N=80, cx=DEFAULT_CENTER_X, cy=DEFAULT_CENTER_Y):
...
def pattern_butterfly(N=60, scale=0.3, cx=DEFAULT_CENTER_X, cy=DEFAULT_CENTER_Y):
    t = np.linspace(0, 24*np.pi, N)
    r = scale * (np.exp(np.cos(t)) - 2*np.cos(4*t) + np.sin(t/12)**5)
    return r * np.sin(t) + cx, r * np.cos(t) + cy / 2

def pattern_clover(N=50, knots=4, scale=0.7, cx=DEFAULT_CENTER_X, cy=DEFAULT_CENTER_Y):
    t = np.linspace(0, 2*np.pi, N)
    r = scale * np.sin(knots * t)
    return r * np.cos(t) + cx, r * np.sin(t) + cy

def pattern_spiral_archimedes(N=80, turns=5, scale=0.03, cx=DEFAULT_CENTER_X, cy=DEFAULT_CENTER_Y):
...
def assemble_motion(j3_offsets, j5_offsets, base=None):
    ...
    return [[base[0], base[1], np.clip(base[2]+180-j5+j3,0,360), base[3], np.clip(base[4]-90+j3,0,360)]
